<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href=".././view/css/style6.css">
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css">

    <title>Quản lý</title>
</head>
<body>
    <div class="fl">

    <div class="menuad py-3 d-flex flex-column align-items-center">
        <div class="avatar row align-items-end px-5">
            <img src=".././view/images/user-solid.svg" alt="" class="col-2">
            <p class="d-inline-block mb-0 font-weight-bold text-white col-2 "><b>Admin</b></p>
        </div>
        <p class="text-white">Xin chào admin</p>
        <button class="mb-3">
            <a class="text-white dangxuat" href="index.php?act=logout">Đăng Xuất</a>
        </button>
        <div class="tinhnang">
            <ul class="px-0 ">
            <li class="mb-2 noidung"><a class="text-white" href="index.php?act=home">Trang chủ</a></li>
                            <li class="mb-2 noidung"><a class="text-white" href="index.php?act=products">Quản lý sản phẩm</a></li>
                            <li class="mb-2 noidung"><a class="text-white" href="index.php?act=categories">Quản lý danh mục</a></li>
                            <li class="mb-2 noidung"><a class="text-white" href="index.php?act=users">Quản lý user</a></li>
                            <li class="mb-2 noidung"><a class="text-white" href="index.php?act=orders">Thống kê doanh thu</a></li>
                            <li class="mb-2 noidung"><a class="text-white" href="index.php?act=comment">Quản lý bình luận</a></li>
            </ul>
        </div>
        <img src=".././view/images/logo.png" alt="" width="50%">
    </div>
