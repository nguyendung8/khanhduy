...
if(isset($_POST['submit'])) {
     ...
     $checkUsername = "SELECT * from user where name = '".$username."'";
     $result = $conn->query($checkUsername);

     if ($result->num_rows > 0) {
         $row = $result->fetch_assoc();
         $user_id = $row['id'];
         $username_origin = $row['name'];
 
         // Kiểm tra username có tồn tại trong hệ thống không
         if ($username === $username_origin) {
             // Username có tồn tại , cập nhật mật khẩu mới
             $updatePasswordQuery = "UPDATE user SET password = '$new_password' WHERE id = $user_id";
             ...
             $message[] = 'Username không tồn tại trên hệ thống, vui lòng nhập lại';
         }
     } else {
...
<?php
   ...
   <div class="change-password">
    <a href="?act=link_login" class="back"><i class="fa fa-arrow-left" aria-hidden="true"></i> Quay lại</a>
